...
open class ParameterizedString
{
    ...
    public init(format:String, arguements : [CVarArg] = [])
    {
        self.format = format
    ...
    {
        let local = format.localize
        if arguements.isEmpty { return local }
        return String(format:local,  arguments: arguements)
    }
}
